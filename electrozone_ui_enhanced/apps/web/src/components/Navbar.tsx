...
import { Search, Heart, User, ShoppingCart, Sun, Moon } from "lucide-react";
      ...
      <div className="max-w-[1440px] mx-auto px-5 md:px-10">
        <div className="flex items-center gap-6 h-20">
          ...
          <nav className="hidden md:flex gap-6 text-sm">
            <Link to="/" className="text-gold border-b-2 border-gold pb-1 font-medium">
              {t("nav.home")}
            </Link>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <button
              ...
              title={theme === "dark" ? "Mode clair" : "Dark Mode"}
            >
              {theme === "dark" ? <Sun size={20} /> : <Moon size={20} />}
            </button>

            {/* Language Toggle */}
            <div className="flex items-center gap-1 bg-navy-tile border border-edge rounded-full p-1 text-xs">
              <button
            ...
            <button className="p-2 rounded-full text-cloud/80 hover:text-gold hover:bg-white/5 transition">
              <Heart size={20} />
            </button>
            <Link to="/admin" className="p-2 rounded-full text-cloud/80 hover:text-gold hover:bg-white/5 transition">
              <User size={20} />
            </Link>
        ...
        <nav className="md:hidden flex gap-4 overflow-x-auto pb-2 text-xs text-cloud/70">
          {categories.map((c) => (
            <Link key={c.id} to={`/c/${c.slug}`} className="whitespace-nowrap hover:text-gold">
              {t("cat." + c.slug) || c.name}
            </Link>
          ))}
        </nav>
