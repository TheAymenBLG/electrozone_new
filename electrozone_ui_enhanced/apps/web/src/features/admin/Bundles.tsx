  ...
  return (
    <div className="max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-head text-3xl font-bold text-gradient">Packs</h1>
          <p className="text-cloud-muted text-sm mt-1">Composez un pack (ex: Pack Mariage) en quelques clics</p>
        </div>
        <button
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {bundles.map((b) => {
          ...
          return (
            <div key={b.id} className="bg-navy-card border border-edge rounded-2xl p-5 hover:border-gold transition-colors">
              <div className="flex justify-between">
                <h3 className="font-head font-bold text-cloud">{b.name}</h3>
                <div className="flex gap-2">
                  <button onClick={() => setEditing(b)} className="text-gold hover:text-gold-bright">
                    <Pencil size={16} />
                  </button>
                  <button
              ...
              <div className="mt-2 flex items-baseline gap-2">
                <span className="font-bold text-lg text-gold font-mono">{formatDA(price)}</span>
                {total > price && (
...
function BundleDialog({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-navy-card border border-edge rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto text-cloud">
        <div className="flex items-center justify-between p-4 border-b border-edge sticky top-0 bg-navy-card">
          <h2 className="font-head font-bold text-lg text-cloud">{value.id ? "Modifier" : "Nouveau"} pack</h2>
        ...
        <div className="p-4 grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              />
              <div className="mt-2 border border-edge rounded-lg divide-y divide-edge max-h-48 overflow-y-auto">
                {results.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => addItem(p.id)}
                    className="w-full text-left p-2 text-sm text-cloud hover:bg-white/5 flex justify-between"
                  >
                    <span>{p.name}</span>
                    <span className="text-cloud-muted font-mono">{formatDA(p.price)}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="mt-1 border border-edge rounded-lg divide-y divide-edge min-h-[120px]">
              {items.length === 0 && (
              ...
              {items.map((it) => {
                const p = products.find((x) => x.id === it.productId)!;
                return (
                  <div key={it.productId} className="p-2 flex items-center justify-between gap-2 text-sm text-cloud">
                    ...
                    <input
                      ...
                      className="w-14 bg-navy-tile border border-edge rounded px-2 py-1 text-cloud"
                    />
                    <button onClick={() => removeItem(it.productId)} className="text-red-400 hover:text-red-300">
                      <X size={16} />
                    </button>
                  </div>
                );
            ...
            <div className="mt-3 bg-navy-tile border border-edge rounded-lg p-3 text-sm space-y-2">
              <div className="flex justify-between">
                <span className="text-cloud-muted">Total des articles</span>
                <span className="font-semibold text-cloud font-mono">{formatDA(total)}</span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <input
                  ...
                  className="w-28 bg-navy-deep border border-edge rounded px-2 py-1 text-right text-cloud"
                />
              </div>
              {saving > 0 && (
                <div className="flex justify-between text-green-300 font-semibold">
                  <span>Économie client</span>
                  <span className="font-mono">{formatDA(saving)}</span>
                </div>
              )}
            </div>

            <label className="flex items-center gap-2 text-sm mt-3 text-cloud">
              <input
                type="checkbox"
