  ...
  if (!product)
    return (
      <div className="max-w-[1440px] mx-auto px-5 md:px-10 py-16">
        {t("product.notFound")} <Link to="/" className="text-gold underline">{t("product.back")}</Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 md:px-10 py-10">
      <nav className="font-mono text-xs text-cloud-muted mb-6">
        ...
        <Link to={`/c/${product.categorySlug}`} className="hover:text-gold uppercase">
          {t("cat." + product.categorySlug) || product.categorySlug}
        </Link>
      </nav>

      <div className="grid md:grid-cols-2 gap-8">
        <div className="relative bg-navy-tile rounded-2xl border border-edge overflow-hidden flex items-center justify-center">
          <div className="absolute w-2/3 h-2/3 bg-gold/10 blur-[70px] rounded-full" />
          {pr.discountPct > 0 && (
        ...
        <div>
          ...
          <p className="mt-4 font-mono text-sm">
            {product.stock > 0 ? (
              <span className="text-gold">{t("product.inStock")} ({product.stock})</span>
            ) : (
          ...
          <button
            ...
            className="mt-6 w-full sm:w-auto bg-gold text-navy font-mono font-bold px-8 py-4 rounded-full hover:bg-gold-bright hover:scale-[1.02] disabled:opacity-50 flex items-center justify-center gap-2 transition-all shadow-lg shadow-gold/10"
          >
            <ShoppingCart size={18} /> {t("product.addToCart")}
          </button>

          <div className="mt-8 grid grid-cols-3 gap-3 font-mono text-[11px] text-cloud-muted">
            <div className="flex flex-col items-center gap-1.5 text-center"><ShieldCheck className="text-gold" size={20} /> {t("product.original")}</div>
            <div className="flex flex-col items-center gap-1.5 text-center"><Truck className="text-gold" size={20} /> {t("product.delivery58")}</div>
            <div className="flex flex-col items-center gap-1.5 text-center"><Wallet className="text-gold" size={20} /> {t("product.cod")}</div>
          </div>
        </div>
      </div>
    </div>
