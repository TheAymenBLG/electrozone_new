  ...
  if (placed)
    return (
      <div className="max-w-3xl mx-auto px-5 md:px-10 py-24 text-center">
        ...
        <h1 className="font-head font-bold text-2xl text-gold mt-4">{t("cart.success")}</h1>
        <p className="mt-2 text-cloud-muted">{t("cart.successSub")}</p>
      </div>
    );

  return (
    <div className="max-w-3xl mx-auto px-5 md:px-10 py-10">
      <h1 className="font-head font-bold text-3xl mb-6">{t("cart.title")}</h1>
      {rows.length === 0 ? (
        <div className="bg-navy-card border border-edge rounded-xl p-16 text-center text-cloud-muted">{t("cart.empty")}</div>
      ) : (
        <>
          ...
          <div className="flex items-center justify-between mt-5">
            <button onClick={clear} className="font-mono text-xs text-cloud-muted underline">{t("cart.clear")}</button>
            <div className="text-right">
              <p className="font-mono text-xs text-cloud-muted">{t("cart.total")}</p>
