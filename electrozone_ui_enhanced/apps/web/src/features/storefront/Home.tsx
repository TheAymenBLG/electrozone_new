...
import { Zap, Play, ShieldCheck, Truck, Headphones, Tag, ArrowRight, Mail, ArrowDown } from "lucide-react";
...
export default function Home() {
  ...
  const FEATURES = [
    { icon: ShieldCheck, title: t("props.quality"), desc: t("props.qualitySub") },
    { icon: Truck, title: t("props.delivery"), desc: t("props.deliverySub") },
    { icon: Headphones, title: t("props.support"), desc: t("props.supportSub") },
    ...
    <div ref={root}>
        ...
        <div className="max-w-[1280px] mx-auto px-5 md:px-10 grid lg:grid-cols-2 gap-12 items-center py-16 md:py-28">
          <div className="relative z-10">
            <span data-hero className="inline-flex items-center gap-2 bg-gold/10 text-gold border border-gold/30 text-xs font-semibold tracking-wider uppercase rounded-full px-4 py-2 mb-8">
              <Zap size={14} /> {t("home.newCollection")}
            </span>
            <h1 ref={headlineRef} className="font-head font-extrabold text-5xl md:text-7xl leading-[1.03]">
              {t("home.titleMain").split(" ").map((w, i) => (
                <span key={i} className="word inline-block mr-[0.25em]">{w}</span>
              ))}
              <br />
              <span className="word inline-block text-gradient">{t("home.titleGold")}</span>
            </h1>
            ...
            <div data-hero className="mt-9 flex items-center gap-4 flex-wrap">
              ...
              <button className="inline-flex items-center gap-2 px-6 py-3.5 border border-cloud/25 text-cloud font-semibold text-sm rounded-full hover:border-gold hover:text-gold transition-all">
                <Play size={16} /> {t("home.watchVideo")}
              </button>
            </div>
            <div data-hero className="grid grid-cols-3 gap-6 mt-12 pt-8 border-t border-edge">
      ...
      <section data-reveal className="max-w-[1280px] mx-auto px-5 md:px-10 mt-20">
        <div className="mb-10">
          <h2 className="font-head font-extrabold text-3xl md:text-5xl">
            {t("home.categoriesTitle")} <span className="text-gradient">{t("home.categoriesTitleAccent")}</span>
          </h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {BENTO.map((b) => (
            <Link key={b.slug} data-reveal-item to={`/c/${b.slug}`} className={`group relative bg-navy-card border border-edge rounded-2xl overflow-hidden h-60 hover:border-gold card-lift ${b.span}`}>
              {catImg(b.slug) && (
      ...
      <section data-reveal className="max-w-[1280px] mx-auto px-5 md:px-10 mt-20">
        <div className="flex items-end justify-between mb-10">
          <h2 className="font-head font-extrabold text-3xl md:text-5xl">
            {t("home.promotionsTitle")} <span className="text-gradient">{t("home.promotionsTitleAccent")}</span>
          </h2>
      ...
      <section data-reveal className="mt-20 max-w-[1280px] mx-auto px-5 md:px-10">
        <div className="mb-10">
          <h2 className="font-head font-extrabold text-3xl md:text-5xl">
            {t("home.brandsTitle")} <span className="text-gradient">{t("home.brandsTitleAccent")}</span> {t("home.brandsTitleSub")}
          </h2>
        </div>
